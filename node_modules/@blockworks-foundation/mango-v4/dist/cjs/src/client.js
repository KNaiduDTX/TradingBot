...
exports.MangoClient = exports.AccountRetriever = exports.SERUM_SETTLE_FUNDS_CU_LIMIT = exports.PERP_SETTLE_FEES_CU_LIMIT = exports.PERP_SETTLE_PNL_CU_LIMIT = exports.DEFAULT_TOKEN_CONDITIONAL_SWAP_COUNT = void 0;
...
const bank_1 = require("./accounts/bank");
const group_1 = require("./accounts/group");
const mangoAccount_1 = require("./accounts/mangoAccount");
const oracle_1 = require("./accounts/oracle");
const perp_1 = require("./accounts/perp");
const serum3_1 = require("./accounts/serum3");
const clientIxParamBuilder_1 = require("./clientIxParamBuilder");
const constants_1 = require("./constants");
const ids_1 = require("./ids");
const mango_v4_1 = require("./mango_v4");
const I80F48_1 = require("./numbers/I80F48");
const utils_1 = require("./utils");
const rpc_1 = require("./utils/rpc");
const spl_1 = require("./utils/spl");
exports.DEFAULT_TOKEN_CONDITIONAL_SWAP_COUNT = 8;
exports.PERP_SETTLE_PNL_CU_LIMIT = 400000;
exports.PERP_SETTLE_FEES_CU_LIMIT = 20000;
exports.SERUM_SETTLE_FUNDS_CU_LIMIT = 65000;
...
})(AccountRetriever || (exports.AccountRetriever = AccountRetriever = {}));
class MangoClient {
    program;
    programId;
    cluster;
    ...
    async sendAndConfirmTransaction(ixs, opts = {}) {
        ...
        const status = await (0, rpc_1.sendTransaction)(this.program.provider, [...this.prependedGlobalAdditionalInstructions, ...ixs], opts.alts ?? [], {
    ...
    async ixGateSet(group, ixGateParams) {
        const ix = await this.program.methods
            .ixGateSet((0, clientIxParamBuilder_1.buildIxGate)(ixGateParams))
            .accounts({
            group: group.publicKey,
            admin: this.program.provider.wallet.publicKey,
        })
    ...
    async getGroup(groupPk) {
        ...
        const group = group_1.Group.from(groupPk, groupAccount);
    ...
    async getGroupsForCreator(creatorPk) {
        ...
        const groups = (await this.program.account.group.all(filters)).map((tuple) => group_1.Group.from(tuple.publicKey, tuple.account));
    ...
    async getGroupForCreator(creatorPk, groupNum) {
        ...
        const filters = [
            ...
            {
                memcmp: {
                    bytes: bs58_1.default.encode(bbuf),
                    offset: 40,
                },
            },
        ];
    ...
    async getIds(groupPk) {
        switch (this.idsSource) {
            case 'api': {
                try {
                    return await ids_1.Id.fromApi(groupPk);
                }
                catch (e) {
                    console.log(e);
                    return undefined;
    ...
    async tokenEdit(group, mintPk, params) {
        ...
        const ix = await this.program.methods
            ...
            ? new anchor_1.BN(params.netBorrowLimitPerWindowQuote)
    ...
    async tokenForceCloseBorrowsWithToken(group, liqor, liqee, assetTokenIndex, liabTokenIndex, maxLiabTransfer) {
        ...
        const ix = await this.program.methods
            ...
            ? (0, utils_1.toNative)(maxLiabTransfer, liabBank.mintDecimals)
            : utils_1.U64_MAX_BN)
            .accounts({
            group: group.publicKey,
    ...
    async tokenForceWithdraw(group, mangoAccount, tokenIndex) {
        ...
        const ownerAtaTokenAccount = await (0, utils_1.getAssociatedTokenAddress)(bank.mint, mangoAccount.owner, true);
        let alternateOwnerTokenAccount = web3_js_1.PublicKey.default;
        const preInstructions = [];
        const postInstructions = [];
        const ai = await this.connection.getAccountInfo(ownerAtaTokenAccount);
        // ensure withdraws don't fail with missing ATAs
        if (ai == null) {
            preInstructions.push(await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(this.program.provider.wallet.publicKey, mangoAccount.owner, bank.mint));
            // wsol case
            if (bank.mint.equals(spl_1.NATIVE_MINT)) {
                postInstructions.push((0, spl_token_1.createCloseAccountInstruction)(ownerAtaTokenAccount, mangoAccount.owner, mangoAccount.owner));
            }
        }
        else {
            const account = await (0, spl_token_1.unpackAccount)(ownerAtaTokenAccount, ai);
            ...
            if (!account.owner.equals(mangoAccount.owner)) {
                ...
                await (0, spl_token_1.createAccount)(this.connection, this.program.provider.wallet, bank.mint, mangoAccount.owner, kp);
                // wsol case
                if (bank.mint.equals(spl_1.NATIVE_MINT)) {
                    postInstructions.push((0, spl_token_1.createCloseAccountInstruction)(alternateOwnerTokenAccount, mangoAccount.owner, mangoAccount.owner));
                }
            }
        }
        const ix = await this.program.methods
            ...
            alternateOwnerTokenAccount: alternateOwnerTokenAccount.equals(web3_js_1.PublicKey.default)
                ? ownerAtaTokenAccount
    ...
    async tokenDeregister(group, mintPk) {
        const bank = group.getFirstBankByMint(mintPk);
        const adminPk = this.program.provider.wallet.publicKey;
        const dustVaultPk = await (0, utils_1.getAssociatedTokenAddress)(bank.mint, adminPk);
        const ai = await this.program.provider.connection.getAccountInfo(dustVaultPk);
        const preInstructions = [];
        if (!ai) {
            preInstructions.push(await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(adminPk, adminPk, bank.mint));
        }
        const ix = await this.program.methods
            .tokenDeregister()
            .accounts({
    ...
    async getBanksForGroup(group) {
        ...
        ])).map((tuple) => bank_1.Bank.from(tuple.publicKey, tuple.account));
    }
    async getMintInfosForGroup(group) {
        return (await this.program.account.mintInfo.all([
            ...
            return bank_1.MintInfo.from(tuple.publicKey, tuple.account);
        });
    }
    async getMintInfoForTokenIndex(group, tokenIndex) {
        ...
        return (await this.program.account.mintInfo.all([
            ...
            {
                memcmp: {
                    bytes: bs58_1.default.encode(tokenIndexBuf),
                    offset: 40,
                },
            },
        ])).map((tuple) => {
    ...
    async stubOracleCreate(group, mintPk, price) {
        const stubOracle = web3_js_1.Keypair.generate();
        const ix = await this.program.methods
            .stubOracleCreate({ val: I80F48_1.I80F48.fromNumber(price).getData() })
            .accounts({
            group: group.publicKey,
            admin: this.program.provider.wallet.publicKey,
    ...
    async stubOracleSet(group, oraclePk, price) {
        const ix = await this.program.methods
            .stubOracleSet({ val: I80F48_1.I80F48.fromNumber(price).getData() })
            .accounts({
            group: group.publicKey,
            admin: this.program.provider.wallet.publicKey,
            oracle: oraclePk,
    ...
    async getStubOracle(group, mintPk) {
        ...
        return (await this.program.account.stubOracle.all(filters)).map((pa) => oracle_1.StubOracle.from(pa.publicKey, pa.account));
    }
    // MangoAccount
    ...
    async editMangoAccount(group, mangoAccount, name, delegate, temporaryDelegate, delegateExpiry) {
        const ix = await this.program.methods
            .accountEdit(name ?? null, delegate ?? null, temporaryDelegate ?? null, delegateExpiry ? new anchor_1.BN(delegateExpiry) : null)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            owner: this.program.provider.wallet.publicKey,
    ...
    getMangoAccountFromAi(mangoAccountPk, ai) {
        ...
        return mangoAccount_1.MangoAccount.from(mangoAccountPk, decodedMangoAccount);
    }
    ...
    async getAllMangoAccounts(group, loadSerum3Oo = false) {
        ...
        if (loadSerum3Oo) {
            const ooPks = accounts
                ...
                .flat();
            const ais = (await Promise.all((0, chunk_1.default)(ooPks, 100).map(async (ooPksChunk) => await this.program.provider.connection.getMultipleAccountsInfo(ooPksChunk)))).flat();
            ...
            const serum3OosMapByOo = new Map(Array.from(ais.map((ai, i) => {
                ...
                const oo = serum_1.OpenOrders.fromAccountInfo(ooPks[i], ai, constants_1.OPENBOOK_PROGRAM_ID[this.cluster]);
    ...
    async emptyAndCloseMangoAccount(group, mangoAccount) {
        ...
        const clonedMangoAccount = (0, fast_copy_1.default)(mangoAccount);
        const instructions = [];
        for (const serum3Account of clonedMangoAccount.serum3Active()) {
            ...
            serum3Account.marketIndex =
                mangoAccount_1.Serum3Orders.Serum3MarketIndexUnset;
        }
        for (const pp of clonedMangoAccount.perpActive()) {
            ...
            pp.marketIndex = mangoAccount_1.PerpPosition.PerpMarketIndexUnset;
        }
        for (const tp of clonedMangoAccount.tokensActive()) {
            const bank = group.getFirstBankByTokenIndex(tp.tokenIndex);
            const withdrawIx = await this.tokenWithdrawNativeIx(group, clonedMangoAccount, bank.mint, utils_1.U64_MAX_BN, false);
            instructions.push(...withdrawIx);
            tp.tokenIndex = mangoAccount_1.TokenPosition.TokenIndexUnset;
        }
    ...
    async accountBuybackFeesWithMngoIx(group, mangoAccount, maxBuybackUsd) {
        ...
        return await this.program.methods
            .accountBuybackFeesWithMngo((0, utils_1.toNative)(maxBuybackUsd, 6))
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
    ...
    async tokenDeposit(group, mangoAccount, mintPk, amount, reduceOnly = false) {
        ...
        const nativeAmount = (0, utils_1.toNative)(amount, decimals);
    ...
    async tokenDepositNative(group, mangoAccount, mintPk, nativeAmount, reduceOnly = false, intoExisting = false) {
        ...
        const tokenAccountPk = await (0, utils_1.getAssociatedTokenAddress)(mintPk, walletPk, 
        // Allow ATA authority to be a PDA
        true);
        let wrappedSolAccount;
        let preInstructions = [];
        let postInstructions = [];
        if (mintPk.equals(spl_1.NATIVE_MINT)) {
            ...
            wrappedSolAccount = await web3_js_1.PublicKey.createWithSeed(walletPk, seed, spl_1.TOKEN_PROGRAM_ID);
            const lamports = nativeAmount.add(new anchor_1.BN(1e7));
            preInstructions = [
                web3_js_1.SystemProgram.createAccountWithSeed({
                    ...
                    space: 165,
                    programId: spl_1.TOKEN_PROGRAM_ID,
                }),
                (0, spl_token_1.createInitializeAccount3Instruction)(wrappedSolAccount, spl_1.NATIVE_MINT, walletPk),
            ];
            postInstructions = [
                (0, spl_token_1.createCloseAccountInstruction)(wrappedSolAccount, walletPk, walletPk),
            ];
        }
        ...
        if (!intoExisting) {
            ixBase = this.program.methods
                .tokenDeposit(new anchor_1.BN(nativeAmount), reduceOnly)
                .accounts({
            ...
            ixBase = this.program.methods
                .tokenDepositIntoExisting(new anchor_1.BN(nativeAmount), reduceOnly)
    ...
    async tokenWithdrawAllDepositForAllUnconfidentOrStaleOracles(group, mangoAccount) {
        ...
        const ixs = await Promise.all(banksToWithdrawFrom.map((bank) => {
            return this.tokenWithdrawNativeIx(group, mangoAccount, bank.mint, utils_1.U64_MAX_BN, false);
        }));
    ...
    async tokenWithdrawAllDepositForMint(group, mangoAccount, mintPk) {
        ...
        const ixs = await this.tokenWithdrawNativeIx(group, mangoAccount, mintPk, utils_1.U64_MAX_BN, false);
    ...
    async tokenWithdraw(group, mangoAccount, mintPk, amount, allowBorrow) {
        const nativeAmount = (0, utils_1.toNative)(amount, group.getMintDecimals(mintPk));
    ...
    async tokenWithdrawNativeIx(group, mangoAccount, mintPk, nativeAmount, allowBorrow) {
        const bank = group.getFirstBankByMint(mintPk);
        const tokenAccountPk = await (0, utils_1.getAssociatedTokenAddress)(bank.mint, mangoAccount.owner, true);
        // ensure withdraws don't fail with missing ATAs
        const preInstructions = [
            await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(mangoAccount.owner, mangoAccount.owner, bank.mint),
        ];
        const postInstructions = [];
        if (mintPk.equals(spl_1.NATIVE_MINT)) {
            postInstructions.push((0, spl_token_1.createCloseAccountInstruction)(tokenAccountPk, mangoAccount.owner, mangoAccount.owner));
        }
        const healthRemainingAccounts = await this.buildHealthRemainingAccounts(group, [mangoAccount], [bank], [], []);
        const ix = await this.program.methods
            .tokenWithdraw(new anchor_1.BN(nativeAmount), allowBorrow)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            owner: mangoAccount.owner,
    ...
    async serum3RegisterMarket(group, serum3MarketExternalPk, baseBank, quoteBank, marketIndex, name, oraclePriceBand) {
        const ix = await this.program.methods
            ...
            group: group.publicKey,
            admin: this.program.provider.wallet.publicKey,
            serumProgram: constants_1.OPENBOOK_PROGRAM_ID[this.cluster],
    ...
    async serum3ConsumeEvents(group, serum3MarketExternalPk) {
        ...
        const eq = (0, serum_1.decodeEventQueue)(ai.data);
    ...
    async serum3deregisterMarket(group, externalMarketPk) {
        ...
        const [indexReservation] = await web3_js_1.PublicKey.findProgramAddress([Buffer.from('Serum3Index'), group.publicKey.toBuffer(), marketIndexBuf], this.program.programId);
        const ix = await this.program.methods
    ...
    async serum3GetMarkets(group, baseTokenIndex, quoteTokenIndex) {
        ...
        if (quoteTokenIndex) {
            ...
            filters.push({
                memcmp: {
                    bytes: bs58_1.default.encode(qbuf),
                    offset: 42,
                },
            });
        }
        return (await this.program.account.serum3Market.all(filters)).map((tuple) => serum3_1.Serum3Market.from(tuple.publicKey, tuple.account));
    }
    ...
    async serum3CloseOpenOrders(group, mangoAccount, externalMarketPk) {
        ...
        return await (0, rpc_1.sendTransaction)(this.program.provider, [ix], group.addressLookupTablesList, {
            postSendTxCallback: this.postSendTxCallback,
            postTxConfirmationCallback: this.postTxConfirmationCallback,
        });
    }
    async serum3LiqForceCancelOrders(group, mangoAccount, externalMarketPk, limit) {
        ...
        const ix = await this.program.methods
            ...
            marketVaultSigner: await (0, serum3_1.generateSerum3MarketExternalVaultSignerAddress)(this.cluster, serum3Market, serum3MarketExternal),
    ...
    async serum3PlaceOrderV1Ix(group, mangoAccount, externalMarketPk, side, price, size, selfTradeBehavior, orderType, clientOrderId, limit) {
        ...
        const serum3MarketExternal = group.serum3ExternalMarketsMap.get(externalMarketPk.toBase58());
        const serum3MarketExternalVaultSigner = await (0, serum3_1.generateSerum3MarketExternalVaultSignerAddress)(this.cluster, serum3Market, serum3MarketExternal);
        ...
        const isTaker = orderType !== serum3_1.Serum3OrderType.postOnly;
        const maxQuoteQuantity = new anchor_1.BN(Math.ceil(serum3MarketExternal.decoded.quoteLotSize.toNumber() *
        ...
        const payerTokenIndex = (() => {
            if (side == serum3_1.Serum3Side.bid) {
        ...
        const ix = await this.program.methods
            .serum3PlaceOrder(side, limitPrice, maxBaseQuantity, maxQuoteQuantity, selfTradeBehavior, orderType, new anchor_1.BN(clientOrderId), limit)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            owner: this.program.provider.wallet.publicKey,
    ...
    async serum3PlaceOrderV2Ix(group, mangoAccount, externalMarketPk, side, price, size, selfTradeBehavior, orderType, clientOrderId, limit) {
        ...
        const ix = await this.program.methods
            .serum3PlaceOrderV2(side, limitPrice, maxBaseQuantity, maxQuoteQuantity, selfTradeBehavior, orderType, new anchor_1.BN(clientOrderId), limit)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            owner: this.program.provider.wallet.publicKey,
    ...
    async serum3SettleFundsV2Ix(group, mangoAccount, externalMarketPk) {
        ...
        const [serum3MarketExternalVaultSigner, openOrderPublicKey] = await Promise.all([
            (0, serum3_1.generateSerum3MarketExternalVaultSignerAddress)(this.cluster, serum3Market, serum3MarketExternal),
    ...
    async perpCreateMarket(group, oraclePk, perpMarketIndex, name, oracleConfig, baseDecimals, quoteLotSize, baseLotSize, maintBaseAssetWeight, initBaseAssetWeight, maintBaseLiabWeight, initBaseLiabWeight, maintOverallAssetWeight, initOverallAssetWeight, baseLiquidationFee, makerFee, takerFee, feePenalty, minFunding, maxFunding, impactQuantity, groupInsuranceFund, settleFeeFlat, settleFeeAmountThreshold, settleFeeFractionLowHealth, settleTokenIndex, settlePnlLimitFactor, settlePnlLimitWindowSize, positivePnlLiquidationFee, platformLiquidationFee) {
        ...
        const ix = await this.program.methods
            .perpCreateMarket(perpMarketIndex, name, oracleConfig, baseDecimals, new anchor_1.BN(quoteLotSize), new anchor_1.BN(baseLotSize), maintBaseAssetWeight, initBaseAssetWeight, maintBaseLiabWeight, initBaseLiabWeight, maintOverallAssetWeight, initOverallAssetWeight, baseLiquidationFee, makerFee, takerFee, minFunding, maxFunding, new anchor_1.BN(impactQuantity), groupInsuranceFund, feePenalty, settleFeeFlat, settleFeeAmountThreshold, settleFeeFractionLowHealth, settleTokenIndex, settlePnlLimitFactor, new anchor_1.BN(settlePnlLimitWindowSize), positivePnlLiquidationFee, platformLiquidationFee)
            .accounts({
            group: group.publicKey,
            admin: this.program.provider.wallet.publicKey,
            oracle: oraclePk,
    ...
    async perpGetMarkets(group) {
        ...
        return (await this.program.account.perpMarket.all(filters)).map((tuple) => perp_1.PerpMarket.from(tuple.publicKey, tuple.account));
    }
    ...
    async perpCloseAll(group, mangoAccount, slippage = 0.01) {
        ...
        const ixs = await Promise.all(mangoAccount.perpActive().map(async (pa) => {
            ...
            const isLong = pa.basePositionLots.gt(new anchor_1.BN(0));
            return await this.perpPlaceOrderV2Ix(group, mangoAccount, pa.marketIndex, isLong ? perp_1.PerpOrderSide.ask : perp_1.PerpOrderSide.bid, pm.uiPrice * (isLong ? 1 - slippage : 1 + slippage), // Try to cross the spread to guarantee matching
            Math.abs(pa.getBasePositionUi(pm) * 1.01), // Send a larger size to ensure full order is closed
            undefined, Date.now(), perp_1.PerpOrderType.immediateOrCancel, perp_1.PerpSelfTradeBehavior.decrementTake, true, // Reduce only
            undefined, undefined);
        }));
    ...
    async perpPlaceOrder(group, mangoAccount, perpMarketIndex, side, price, quantity, maxQuoteQuantity, clientOrderId, orderType, reduceOnly, expiryTimestamp, limit) {
        const ix = await this.perpPlaceOrderV2Ix(group, mangoAccount, perpMarketIndex, side, price, quantity, maxQuoteQuantity, clientOrderId, orderType, perp_1.PerpSelfTradeBehavior.decrementTake, reduceOnly, expiryTimestamp, limit);
        return await this.sendAndConfirmTransactionForGroup(group, [ix]);
    }
    async perpPlaceOrderIx(group, mangoAccount, perpMarketIndex, side, price, quantity, maxQuoteQuantity, clientOrderId, orderType, reduceOnly, expiryTimestamp, limit) {
        ...
        return await this.program.methods
            ...
            : utils_1.I64_MAX_BN, new anchor_1.BN(clientOrderId ? clientOrderId : Date.now()), orderType ? orderType : perp_1.PerpOrderType.limit, reduceOnly ? reduceOnly : false, new anchor_1.BN(expiryTimestamp ? expiryTimestamp : 0), limit ? limit : 10)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            perpMarket: perpMarket.publicKey,
    ...
    async perpPlaceOrderV2Ix(group, mangoAccount, perpMarketIndex, side, price, quantity, maxQuoteQuantity, clientOrderId, orderType, selfTradeBehavior, reduceOnly, expiryTimestamp, limit) {
        ...
        return await this.program.methods
            ...
            ? perpMarket.uiQuoteToLots(maxQuoteQuantity)
            : utils_1.I64_MAX_BN, new anchor_1.BN(clientOrderId ? clientOrderId : Date.now()), orderType ?? perp_1.PerpOrderType.limit, selfTradeBehavior ?? perp_1.PerpSelfTradeBehavior.decrementTake, reduceOnly ?? false, new anchor_1.BN(expiryTimestamp ? expiryTimestamp : 0), limit ?? 10)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            perpMarket: perpMarket.publicKey,
    ...
    async perpPlaceOrderPegged(group, mangoAccount, perpMarketIndex, side, priceOffset, quantity, pegLimit, maxQuoteQuantity, clientOrderId, orderType, reduceOnly, expiryTimestamp, limit) {
        const ix = await this.perpPlaceOrderPeggedV2Ix(group, mangoAccount, perpMarketIndex, side, priceOffset, quantity, pegLimit, maxQuoteQuantity, clientOrderId, orderType, perp_1.PerpSelfTradeBehavior.decrementTake, reduceOnly, expiryTimestamp, limit);
        return await this.sendAndConfirmTransactionForGroup(group, [ix]);
    }
    async perpPlaceOrderPeggedIx(group, mangoAccount, perpMarketIndex, side, priceOffset, quantity, pegLimit, maxQuoteQuantity, clientOrderId, orderType, reduceOnly, expiryTimestamp, limit) {
        ...
        [group.getFirstBankForPerpSettlement()], [perpMarket]);
        return await this.program.methods
            .perpPlaceOrderPegged(side, perpMarket.uiPriceToLots(priceOffset), pegLimit ? perpMarket.uiPriceToLots(pegLimit) : new anchor_1.BN(-1), perpMarket.uiBaseToLots(quantity), maxQuoteQuantity
            ? perpMarket.uiQuoteToLots(maxQuoteQuantity)
            : utils_1.I64_MAX_BN, new anchor_1.BN(clientOrderId ?? Date.now()), orderType ? orderType : perp_1.PerpOrderType.limit, reduceOnly ? reduceOnly : false, new anchor_1.BN(expiryTimestamp ?? 0), limit ? limit : 10, -1)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            perpMarket: perpMarket.publicKey,
    ...
    async perpPlaceOrderPeggedV2Ix(group, mangoAccount, perpMarketIndex, side, priceOffset, quantity, pegLimit, maxQuoteQuantity, clientOrderId, orderType, selfTradeBehavior, reduceOnly, expiryTimestamp, limit) {
        ...
        [group.getFirstBankForPerpSettlement()], [perpMarket]);
        return await this.program.methods
            .perpPlaceOrderPeggedV2(side, perpMarket.uiPriceToLots(priceOffset), pegLimit ? perpMarket.uiPriceToLots(pegLimit) : new anchor_1.BN(-1), perpMarket.uiBaseToLots(quantity), maxQuoteQuantity
            ? perpMarket.uiQuoteToLots(maxQuoteQuantity)
            : utils_1.I64_MAX_BN, new anchor_1.BN(clientOrderId ?? Date.now()), orderType ?? perp_1.PerpOrderType.limit, selfTradeBehavior ?? perp_1.PerpSelfTradeBehavior.decrementTake, reduceOnly ?? false, new anchor_1.BN(expiryTimestamp ?? 0), limit ?? 10, -1)
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            perpMarket: perpMarket.publicKey,
    ...
    async perpCancelOrderByClientOrderIdIx(group, mangoAccount, perpMarketIndex, clientOrderId) {
        const perpMarket = group.getPerpMarketByMarketIndex(perpMarketIndex);
        return await this.program.methods
            .perpCancelOrderByClientOrderId(new anchor_1.BN(clientOrderId))
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            owner: this.program.provider.wallet.publicKey,
    ...
    async perpCancelOrderIx(group, mangoAccount, perpMarketIndex, orderId) {
        const perpMarket = group.getPerpMarketByMarketIndex(perpMarketIndex);
        return await this.program.methods
            .perpCancelOrder(new anchor_1.BN(orderId))
            .accounts({
            group: group.publicKey,
            account: mangoAccount.publicKey,
            owner: this.program.provider.wallet.publicKey,
    ...
    async settleAll(client, group, mangoAccount, allMangoAccounts) {
        ...
        if (mangoAccount.perpActive().length *
            (exports.PERP_SETTLE_PNL_CU_LIMIT + exports.PERP_SETTLE_FEES_CU_LIMIT) +
            mangoAccount.serum3Active().length * exports.SERUM_SETTLE_FUNDS_CU_LIMIT >
            1600000) {
    ...
    async perpSettlePnlAndFees(group, profitableAccount, unprofitableAccount, accountToSettleFeesFor, settler, perpMarketIndex, maxSettleAmount) {
        return await this.sendAndConfirmTransactionForGroup(group, [
            web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({
                units: exports.PERP_SETTLE_PNL_CU_LIMIT + exports.PERP_SETTLE_FEES_CU_LIMIT,
            }),
    ...
    async perpSettlePnl(group, profitableAccount, unprofitableAccount, settler, perpMarketIndex) {
        return await this.sendAndConfirmTransactionForGroup(group, [
            web3_js_1.ComputeBudgetProgram.setComputeUnitLimit({
                units: exports.PERP_SETTLE_PNL_CU_LIMIT,
            }),
            await this.perpSettlePnlIx(group, profitableAccount, unprofitableAccount, settler, perpMarketIndex),
        ]);
    }
    ...
    async perpSettleFeesIx(group, account, perpMarketIndex, maxSettleAmount) {
        ...
        const bank = group.banksMapByTokenIndex.get(0)[0];
        return await this.program.methods
            .perpSettleFees(maxSettleAmount ? (0, utils_1.toNative)(maxSettleAmount, 6) : (0, constants_1.RUST_U64_MAX)())
            .accounts({
            group: group.publicKey,
            account: account.publicKey,
            perpMarket: perpMarket.publicKey,
    ...
    async perpConsumeEventsIx(group, perpMarketIndex, accounts, limit) {
        const perpMarket = group.getPerpMarketByMarketIndex(perpMarketIndex);
        return await this.program.methods
            .perpConsumeEvents(new anchor_1.BN(limit))
            .accounts({
            group: group.publicKey,
            perpMarket: perpMarket.publicKey,
            eventQueue: perpMarket.eventQueue,
    ...
    async perpConsumeAllEvents(group, perpMarketIndex) {
        ...
        while (unconsumedEvents.length > 0) {
            ...
            const accounts = events
                .map((ev) => {
                switch (ev.eventType) {
                    case perp_1.PerpEventQueue.FILL_EVENT_TYPE: {
                        const fill = ev;
                        return [fill.maker, fill.taker];
                    }
                    case perp_1.PerpEventQueue.OUT_EVENT_TYPE: {
                        const out = ev;
                        return [out.owner];
                    }
                    case perp_1.PerpEventQueue.LIQUIDATE_EVENT_TYPE:
                        return [];
                    default:
    ...
    flashLoanType, sequenceCheck = false, }) {
         ...
         */
        const inputTokenAccountPk = await (0, utils_1.getAssociatedTokenAddress)(inputBank.mint, swapExecutingWallet, true);
        ...
        if (!inputTokenAccExists) {
            preInstructions.push(await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(swapExecutingWallet, swapExecutingWallet, inputBank.mint));
        }
        const outputTokenAccountPk = await (0, utils_1.getAssociatedTokenAddress)(outputBank.mint, swapExecutingWallet, true);
        ...
        if (!outputTokenAccExists) {
            preInstructions.push(await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(swapExecutingWallet, swapExecutingWallet, outputBank.mint));
        }
        ...
        const flashLoanBeginIx = await this.program.methods
            .flashLoanBegin([
            (0, utils_1.toNative)(amountIn, inputBank.mintDecimals),
            new anchor_1.BN(0) /* we don't care about borrowing the target amount, this is just a dummy */,
        ])
            .accounts({
            account: mangoAccount.publicKey,
            owner: this.program.provider.wallet.publicKey,
    ...
    async liqTokenWithToken(group, liqor, liqee, assetMintPk, liabMintPk, maxLiabTransfer) {
        ...
        const ix = await this.program.methods
            ...
            val: I80F48_1.I80F48.fromNumber(maxLiabTransfer).getData(),
        })
            .accounts({
            group: group.publicKey,
            liqor: liqor.publicKey,
    ...
    async tcsTakeProfitOnDepositIx(group, account, sellBank, buyBank, thresholdPrice, thresholdPriceInSellPerBuyToken, maxSell, pricePremium, expiryTimestamp) {
        ...
        const thresholdPriceNativeNative = (0, utils_1.toNativeSellPerBuyTokenPrice)(thresholdPrice, sellBank, buyBank);
        const lowerLimit = 0;
    ...
    async tokenConditionalSwapCreateIx(group, account, sellBank, buyBank, lowerLimitNativeNative, upperLimitNativeNative, maxBuy, maxSell, tcsIntention, pricePremium, allowCreatingDeposits, allowCreatingBorrows, expiryTimestamp, displayPriceInSellTokenPerBuyToken) {
        const maxBuyNative = maxBuy == Number.MAX_SAFE_INTEGER
            ? utils_1.U64_MAX_BN
            : (0, utils_1.toNative)(maxBuy, buyBank.mintDecimals);
        const maxSellNative = maxSell == Number.MAX_SAFE_INTEGER
            ? utils_1.U64_MAX_BN
            : (0, utils_1.toNative)(maxSell, sellBank.mintDecimals);
        pricePremium = mangoAccount_1.TokenConditionalSwap.computePremium(group, buyBank, sellBank, maxBuyNative, maxSellNative, maxBuy, maxSell);
        const pricePremiumRate = pricePremium / 100;
        let intention;
        switch (tcsIntention) {
            case 'StopLossOnBorrow':
            case 'StopLossOnDeposit':
                intention = mangoAccount_1.TokenConditionalSwapIntention.stopLoss;
                break;
            case 'TakeProfitOnBorrow':
            case 'TakeProfitOnDeposit':
                intention = mangoAccount_1.TokenConditionalSwapIntention.takeProfit;
                break;
            default:
                intention = mangoAccount_1.TokenConditionalSwapIntention.unknown;
                break;
        }
        return await this.tokenConditionalSwapCreateRawIx(group, account, buyBank.mint, sellBank.mint, maxBuyNative, maxSellNative, expiryTimestamp, lowerLimitNativeNative, upperLimitNativeNative, pricePremiumRate, allowCreatingDeposits, allowCreatingBorrows, displayPriceInSellTokenPerBuyToken
            ? mangoAccount_1.TokenConditionalSwapDisplayPriceStyle.sellTokenPerBuyToken
            : mangoAccount_1.TokenConditionalSwapDisplayPriceStyle.buyTokenPerSellToken, intention);
    }
    ...
    async tokenConditionalSwapCreateLinearAuctionIx(group, account, sellBank, buyBank, priceStart, priceEnd, maxBuy, maxSell, allowCreatingDeposits, allowCreatingBorrows, displayPriceInSellTokenPerBuyToken, startTimestamp, durationSeconds, expiryTimestamp) {
        ...
        if (maxBuy == Number.MAX_SAFE_INTEGER) {
            maxBuyNative = utils_1.U64_MAX_BN;
        }
        else {
            maxBuyNative = (0, utils_1.toNative)(maxBuy, buyBank.mintDecimals);
        }
        if (maxSell == Number.MAX_SAFE_INTEGER) {
            maxSellNative = utils_1.U64_MAX_BN;
        }
        else {
            maxSellNative = (0, utils_1.toNative)(maxSell, sellBank.mintDecimals);
        }
        const priceStartNative = (0, utils_1.toNativeSellPerBuyTokenPrice)(priceStart, sellBank, buyBank);
        const priceEndNative = (0, utils_1.toNativeSellPerBuyTokenPrice)(priceEnd, sellBank, buyBank);
        const tcsIx = await this.program.methods
            .tokenConditionalSwapCreateLinearAuction(maxBuyNative, maxSellNative, expiryTimestamp !== null ? new anchor_1.BN(expiryTimestamp) : utils_1.U64_MAX_BN, priceStartNative, priceEndNative, allowCreatingDeposits, allowCreatingBorrows, displayPriceInSellTokenPerBuyToken
            ? mangoAccount_1.TokenConditionalSwapDisplayPriceStyle.sellTokenPerBuyToken
            : mangoAccount_1.TokenConditionalSwapDisplayPriceStyle.buyTokenPerSellToken, new anchor_1.BN(startTimestamp), new anchor_1.BN(durationSeconds))
            .accounts({
            group: group.publicKey,
            account: account.publicKey,
        ...
        if (account.tokenConditionalSwaps.length == 0) {
            ixs.push(await this.accountExpandV2Ix(group, account, account.tokens.length, account.serum3.length, account.perps.length, account.perpOpenOrders.length, exports.DEFAULT_TOKEN_CONDITIONAL_SWAP_COUNT));
        }
        ixs.push(tcsIx);
        return ixs;
    }
    ...
    async tokenConditionalSwapCreatePremiumAuctionIx(group, account, sellBank, buyBank, lowerLimitNative, upperLimitNative, maxBuy, maxSell, tcsIntention, maxPricePremiumPercent, allowCreatingDeposits, allowCreatingBorrows, expiryTimestamp, displayPriceInSellTokenPerBuyToken, durationSeconds, premiumMultiplier = 1, extraPricePremiumBps = 0) {
        ...
        const tcsIx = await this.program.methods
            .tokenConditionalSwapCreatePremiumAuction(maxBuyNative, maxSellNative, expiryTimestamp !== null ? new anchor_1.BN(expiryTimestamp) : utils_1.U64_MAX_BN, lowerLimitNative, upperLimitNative, maxPricePremiumRate, allowCreatingDeposits, allowCreatingBorrows, displayPriceInSellTokenPerBuyToken
            ? mangoAccount_1.TokenConditionalSwapDisplayPriceStyle.sellTokenPerBuyToken
            : mangoAccount_1.TokenConditionalSwapDisplayPriceStyle.buyTokenPerSellToken, intention, new anchor_1.BN(durationSeconds))
            .accounts({
            group: group.publicKey,
            account: account.publicKey,
            authority: this.program.provider.wallet.publicKey,
    ...
    async tokenConditionalSwapCreateRawIx(group, account, buyMintPk, sellMintPk, maxBuy, maxSell, expiryTimestamp, priceLowerLimit, priceUpperLimit, pricePremiumRate, allowCreatingDeposits, allowCreatingBorrows, priceDisplayStyle, intention) {
        ...
        const tcsIx = await this.program.methods
            .tokenConditionalSwapCreateV2(maxBuy, maxSell, expiryTimestamp !== null ? new anchor_1.BN(expiryTimestamp) : utils_1.U64_MAX_BN, priceLowerLimit, priceUpperLimit, pricePremiumRate, allowCreatingDeposits, allowCreatingBorrows, priceDisplayStyle, intention)
            .accounts({
            group: group.publicKey,
            account: account.publicKey,
            authority: this.program.provider.wallet.publicKey,
    ...
    async tokenConditionalSwapCancelIx(group, account, tokenConditionalSwapId) {
        ...
        return this.program.methods
            .tokenConditionalSwapCancel(tokenConditionalSwapIndex, new anchor_1.BN(tokenConditionalSwapId))
            .accounts({
            group: group.publicKey,
            account: account.publicKey,
            authority: this.program.provider.wallet.publicKey,
    ...
    async tokenConditionalSwapCancelAll(group, account) {
        const ixs = await Promise.all(account.tokenConditionalSwaps
            ...
            const sellBank = group.banksMapByTokenIndex.get(tcs.sellTokenIndex)[0];
            return await this.program.methods
                .tokenConditionalSwapCancel(i, new anchor_1.BN(tcs.id))
                .accounts({
                group: group.publicKey,
                account: account.publicKey,
    ...
    async tokenConditionalSwapTriggerIx(group, liqee, liqor, tokenConditionalSwapId, maxBuyTokenToLiqee, maxSellTokenToLiqor) {
        ...
        return this.program.methods
            .tokenConditionalSwapTrigger(tokenConditionalSwapIndex, new anchor_1.BN(tokenConditionalSwapId), new anchor_1.BN(maxBuyTokenToLiqee), new anchor_1.BN(maxSellTokenToLiqor))
            .accounts({
            group: group.publicKey,
            liqee: liqee.publicKey,
            liqor: liqor.publicKey,
    ...
    static connect(provider, cluster, programId, opts) {
        const idl = mango_v4_1.IDL;
        console.log(opts);
        return new MangoClient(new anchor_1.Program(idl, programId, provider), programId, cluster, opts);
    }
    /**
    ...
    static connectDefault(clusterUrl) {
        ...
        const connection = new web3_js_1.Connection(clusterUrl, options);
        return new MangoClient(new anchor_1.Program(idl, constants_1.MANGO_V4_ID['mainnet-beta'], new anchor_1.AnchorProvider(connection, new anchor_1.Wallet(new web3_js_1.Keypair()), options)), constants_1.MANGO_V4_ID['mainnet-beta'], 'mainnet-beta', {
            idsSource: 'get-program-accounts',
        });
    }
    static connectForGroupName(provider, groupName) {
        const idl = mango_v4_1.IDL;
        const id = ids_1.Id.fromIdsByName(groupName);
        return new MangoClient(new anchor_1.Program(idl, new web3_js_1.PublicKey(id.mangoProgramId), provider), new web3_js_1.PublicKey(id.mangoProgramId), id.cluster);
    }
    /**
    ...
    banks = [], perpMarkets = [], openOrdersForMarket = []) {
        ...
        for (const bank of banks) {
            ...
            if (!tokenPositionExists) {
                const inactiveTokenPosition = tokenPositionIndices.findIndex((index) => index === mangoAccount_1.TokenPosition.TokenIndexUnset);
        ...
        const mintInfos = (0, uniq_1.default)(tokenPositionIndices
            .filter((tokenIndex) => tokenIndex !== mangoAccount_1.TokenPosition.TokenIndexUnset)
        ...
        for (const perpMarket of perpMarkets) {
            ...
            if (!perpPositionExists) {
                const inactivePerpPosition = perpPositionsMarketIndices.findIndex((perpIdx) => perpIdx === mangoAccount_1.PerpPosition.PerpMarketIndexUnset);
        ...
        const allPerpMarkets = (0, uniq_1.default)(perpPositionsMarketIndices
            .filter((perpMarktIndex) => perpMarktIndex !== mangoAccount_1.PerpPosition.PerpMarketIndexUnset)
        ...
        for (const [serum3Market, openOrderPk] of openOrdersForMarket) {
            ...
            if (!ooPositionExists) {
                const inactiveSerumPosition = serumPositionMarketIndices.findIndex((serumPos) => serumPos.marketIndex === mangoAccount_1.Serum3Orders.Serum3MarketIndexUnset);
        ...
        healthRemainingAccounts.push(...serumPositionMarketIndices
            .filter((serumPosition) => serumPosition.marketIndex !== mangoAccount_1.Serum3Orders.Serum3MarketIndexUnset)
        ...
        for (const fallback of (0, uniq_1.default)(fallbacks)) {
            if (!healthRemainingAccounts.find((h) => h.equals(fallback)) &&
                !fallback.equals(web3_js_1.PublicKey.default)) {
    ...
    async deriveFallbackOracleContexts(group) {
        ...
        if (typeof this.fallbackOracleConfig !== 'string') {
            if (this.fixedFallbacks.size === 0) {
                ...
                this.fixedFallbacks = await (0, oracle_1.createFallbackOracleMap)(this.connection, oracles, fallbacks);
            }
        ...
        switch (this.fallbackOracleConfig) {
            ...
            case 'dynamic': {
                ...
                return (0, oracle_1.createFallbackOracleMap)(this.connection, oracles, fallbacks);
            }
            case 'all': {
                const oracles = [];
                const fallbacks = [];
    ...
    async estimatePrioritizationFee(ixs) {
        const writableAccounts = ixs
            ...
            .flat();
        const uniqueWritableAccounts = (0, uniq_1.default)(writableAccounts.map((x) => x.toBase58()))
            .map((a) => new web3_js_1.PublicKey(a))
            .slice(0, constants_1.MAX_RECENT_PRIORITY_FEE_ACCOUNTS);
        ...
        const maxFeeBySlot = (0, mapValues_1.default)((0, groupBy_1.default)(priorityFees, 'slot'), (items) => (0, maxBy_1.default)(items, 'prioritizationFee'));
