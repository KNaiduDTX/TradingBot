...
exports.MintInfo = exports.Bank = void 0;
...
class Bank {
    ...
    constructor(publicKey, group, name, mint, vault, oracle, oracleConfig, stablePriceModel, depositIndex, borrowIndex, indexedDeposits, indexedBorrows, indexLastUpdated, bankRateLastUpdated, avgUtilization, adjustmentFactor, util0, rate0, util1, rate1, maxRate, collectedFeesNative, loanOriginationFeeRate, loanFeeRate, maintAssetWeight, initAssetWeight, maintLiabWeight, initLiabWeight, liquidationFee, dust, flashLoanTokenAccountInitial, flashLoanApprovedAmount, tokenIndex, mintDecimals, bankNum, minVaultToDepositsRatio, netBorrowLimitWindowSizeTs, lastNetBorrowsWindowStartTs, netBorrowLimitPerWindowQuote, netBorrowsInWindow, borrowWeightScaleStartQuote, depositWeightScaleStartQuote, reduceOnly, forceClose, feesWithdrawn, tokenConditionalSwapTakerFeeRate, tokenConditionalSwapMakerFeeRate, flashLoanSwapFeeRate, interestTargetUtilization, interestCurveScaling, potentialSerumTokens, maintWeightShiftStart, maintWeightShiftEnd, maintWeightShiftDurationInv, maintWeightShiftAssetTarget, maintWeightShiftLiabTarget, fallbackOracle, depositLimit, zeroUtilRate, platformLiquidationFee, collectedLiquidationFees, allowAssetLiquidation, collectedCollateralFees, collateralFeePerDay, tier, forceWithdraw) {
        ...
        this.depositIndex = I80F48_1.I80F48.from(depositIndex);
        this.borrowIndex = I80F48_1.I80F48.from(borrowIndex);
        this.indexedDeposits = I80F48_1.I80F48.from(indexedDeposits);
        this.indexedBorrows = I80F48_1.I80F48.from(indexedBorrows);
        this.avgUtilization = I80F48_1.I80F48.from(avgUtilization);
        this.adjustmentFactor = I80F48_1.I80F48.from(adjustmentFactor);
        this.maxRate = I80F48_1.I80F48.from(maxRate);
        this.util0 = I80F48_1.I80F48.from(util0);
        this.rate0 = I80F48_1.I80F48.from(rate0);
        this.util1 = I80F48_1.I80F48.from(util1);
        this.rate1 = I80F48_1.I80F48.from(rate1);
        this.collectedFeesNative = I80F48_1.I80F48.from(collectedFeesNative);
        this.loanFeeRate = I80F48_1.I80F48.from(loanFeeRate);
        this.loanOriginationFeeRate = I80F48_1.I80F48.from(loanOriginationFeeRate);
        this.maintAssetWeight = I80F48_1.I80F48.from(maintAssetWeight);
        this.initAssetWeight = I80F48_1.I80F48.from(initAssetWeight);
        this.maintLiabWeight = I80F48_1.I80F48.from(maintLiabWeight);
        this.initLiabWeight = I80F48_1.I80F48.from(initLiabWeight);
        this.liquidationFee = I80F48_1.I80F48.from(liquidationFee);
        this.dust = I80F48_1.I80F48.from(dust);
        this.maintWeightShiftDurationInv = I80F48_1.I80F48.from(maintWeightShiftDurationInv);
        this.maintWeightShiftAssetTarget = I80F48_1.I80F48.from(maintWeightShiftAssetTarget);
        this.maintWeightShiftLiabTarget = I80F48_1.I80F48.from(maintWeightShiftLiabTarget);
        this.zeroUtilRate = I80F48_1.I80F48.from(zeroUtilRate);
        this.platformLiquidationFee = I80F48_1.I80F48.from(platformLiquidationFee);
        this.collectedLiquidationFees = I80F48_1.I80F48.from(collectedLiquidationFees);
        this.collectedCollateralFees = I80F48_1.I80F48.from(collectedCollateralFees);
        this._price = undefined;
        this._uiPrice = undefined;
        this._oracleLastUpdatedSlot = undefined;
        this._oracleProvider = undefined;
        this.tier = bytes_1.utf8.decode(new Uint8Array(tier)).split('\x00')[0];
    }
    toString() {
        return ('Bank ' +
            '\n public key - ' +
    ...
    scaledInitAssetWeight(price) {
        const depositsQuote = this.nativeDeposits()
            .add(I80F48_1.I80F48.fromU64(this.potentialSerumTokens))
            .mul(price);
        if (this.depositWeightScaleStartQuote >= Number.MAX_SAFE_INTEGER ||
            depositsQuote.lte(I80F48_1.I80F48.fromNumber(this.depositWeightScaleStartQuote))) {
            return this.initAssetWeight;
        }
        return this.initAssetWeight.mul(I80F48_1.I80F48.fromNumber(this.depositWeightScaleStartQuote).div(depositsQuote));
    }
    scaledInitLiabWeight(price) {
        ...
        if (this.borrowWeightScaleStartQuote >= Number.MAX_SAFE_INTEGER ||
            borrowsQuote.lte(I80F48_1.I80F48.fromNumber(this.borrowWeightScaleStartQuote))) {
            return this.initLiabWeight;
        }
        return this.initLiabWeight.mul(borrowsQuote.div(I80F48_1.I80F48.fromNumber(this.borrowWeightScaleStartQuote)));
    }
    maintWeights() {
        ...
        if (this.maintWeightShiftDurationInv.isZero() ||
            ...
            const scale = I80F48_1.I80F48.fromU64(nowTs.sub(this.maintWeightShiftStart)).mul(this.maintWeightShiftDurationInv);
    ...
    getAssetPrice() {
        return this.price.min(I80F48_1.I80F48.fromNumber(this.stablePriceModel.stablePrice));
    }
    getLiabPrice() {
        return this.price.max(I80F48_1.I80F48.fromNumber(this.stablePriceModel.stablePrice));
    }
    get price() {
    ...
    uiDeposits() {
        return (0, utils_1.toUiDecimals)(this.nativeDeposits(), this.mintDecimals);
    }
    uiBorrows() {
        return (0, utils_1.toUiDecimals)(this.nativeBorrows(), this.mintDecimals);
    }
    /**
     *
    ...
    getBorrowRateWithoutUpkeepRate() {
        ...
        const utilization = totalBorrows
            ...
            .min((0, I80F48_1.ONE_I80F48)());
        const scaling = I80F48_1.I80F48.fromNumber(this.interestCurveScaling == 0.0 ? 1.0 : this.interestCurveScaling);
        if (utilization.lt(this.util0)) {
            ...
            const slope = this.maxRate
                .sub(this.rate1)
                .div(I80F48_1.I80F48.fromNumber(1).sub(this.util1));
    ...
    getNetBorrowLimitPerWindow() {
        return I80F48_1.I80F48.fromI64(this.netBorrowLimitPerWindowQuote).div(this.price);
    }
    getBorrowLimitLeftInWindow() {
        return this.getNetBorrowLimitPerWindow()
            .sub(I80F48_1.I80F48.fromI64(this.netBorrowsInWindow))
            .max((0, I80F48_1.ZERO_I80F48)());
    }
    getNetBorrowLimitPerWindowUi() {
        return (0, utils_1.toUiDecimals)(this.getNetBorrowLimitPerWindow(), this.mintDecimals);
    }
    getMaxWithdraw(vaultBalance, userDeposits = (0, I80F48_1.ZERO_I80F48)()) {
        userDeposits = userDeposits.max((0, I80F48_1.ZERO_I80F48)());
        // any borrow must respect the minVaultToDepositsRatio
        const minVaultBalanceRequired = this.nativeDeposits().mul(I80F48_1.I80F48.fromNumber(this.minVaultToDepositsRatio));
        const maxBorrowFromVault = I80F48_1.I80F48.fromI64(vaultBalance)
            .sub(minVaultBalanceRequired)
            .max((0, I80F48_1.ZERO_I80F48)());
        // User deposits can exceed maxWithdrawFromVault
        let maxBorrow = maxBorrowFromVault.sub(userDeposits).max((0, I80F48_1.ZERO_I80F48)());
        ...
        maxBorrow = maxBorrow.div((0, I80F48_1.ONE_I80F48)().add(this.loanOriginationFeeRate));
        // user deposits can always be withdrawn
        // even if vaults can be depleted
        return maxBorrow.add(userDeposits).min(I80F48_1.I80F48.fromI64(vaultBalance));
    }
    getTimeToNextBorrowLimitWindowStartsTs() {
        const timeToNextBorrowLimitWindowStartsTs = this.netBorrowLimitWindowSizeTs
            .sub(new anchor_1.BN(Date.now() / 1000).sub(this.lastNetBorrowsWindowStartTs))
            .toNumber();
    ...
    getRemainingDepositLimit() {
        ...
        const remainingDepositLimit = !isNoLimit
            ? this.depositLimit
                .sub(new anchor_1.BN(nativeDeposits.toNumber()))
...
class MintInfo {
    ...
    toString() {
        const res = 'mint ' +
            ...
            '\n banks ' +
            this.banks
                .filter((pk) => pk.toBase58() !== web3_js_1.PublicKey.default.toBase58())
                .toString() +
            '\n vaults ' +
            this.vaults
                .filter((pk) => pk.toBase58() !== web3_js_1.PublicKey.default.toBase58())
