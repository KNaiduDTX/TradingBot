...
export async function sendTransaction(provider, ixs, alts, opts = {}) {
    ...
    const computeUnitLimitIxFound = ixs.some((ix) => ix.programId.equals(COMPUTE_BUDGET_PROGRAM_ID) &&
        u8().decode(ix.data.subarray(0, 1)) == 2);
    if (!computeUnitLimitIxFound) {
        const totalUserIntendedIxs = ixs.filter((ix) => !ix.programId.equals(COMPUTE_BUDGET_PROGRAM_ID)).length;
...
const confirmTransaction = async (connection, opts = {}, latestBlockhash, signature) => {
    ...
    try {
        if (latestBlockhash.blockhash != null &&
            ...
            status = await Promise.any(allConnections.map((c) => awaitTransactionSignatureConfirmation({
                txid: signature,
...
export async function fetchLatestBlockHash(provider, opts = {}) {
...
export const createComputeLimitIx = (units) => {
