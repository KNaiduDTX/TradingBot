...
describe('Mango Account', () => {
    const mangoAccount = new MangoAccount(PublicKey.default, PublicKey.default, PublicKey.default, [], PublicKey.default, 0, false, false, new BN(0), new BN(0), new BN(100), new BN(0), new BN(0), new BN(0), new BN(0), 0, 0, new BN(0), [], [], [], [], [], new Map());
    it('test calculateEquivalentSourceAmount', (done) => {
        const mockedSOLBank = {
            mintDecimals: 9,
            price: I80F48.fromNumber(0.09870201707999726),
        };
        const SOLDepositLimitLeft = new BN(toNative(0.5, mockedSOLBank.mintDecimals));
        const mockedUSDCBank = {
            mintDecimals: 6,
            price: I80F48.fromNumber(1.0000536899999979),
        };
        const USDCDepositLimitLeft = new BN(toNative(1, mockedUSDCBank.mintDecimals));
        ...
        expect(toUiDecimals(maxSourceForUSDCTarget, mockedSOLBank.mintDecimals).toFixed(2) === '0.01').to.be.true;
        // Expected u can buy max of 0.49 SOL for 49 USDC
        expect(toUiDecimals(maxSourceForSOLTarget, mockedUSDCBank.mintDecimals).toFixed(0) === '49').to.be.true;
        done();
    });
});
describe('maxWithdraw', () => {
    const protoAccount = new MangoAccount(PublicKey.default, PublicKey.default, PublicKey.default, [], PublicKey.default, 0, false, false, new BN(0), new BN(0), new BN(0), new BN(0), new BN(0), new BN(0), new BN(0), 0, 0, new BN(0), [], [], [], [], [], new Map());
    protoAccount.tokens.push(new TokenPosition(ZERO_I80F48(), 0, 0, ZERO_I80F48(), 0, 0));
    protoAccount.tokens.push(new TokenPosition(ZERO_I80F48(), 1, 0, ZERO_I80F48(), 0, 0));
    const protoBank = {
    ...
    function setup(vaultAmount) {
        const account = deepClone(protoAccount);
