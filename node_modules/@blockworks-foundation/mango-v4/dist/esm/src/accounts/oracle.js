...
import { IDL, } from './pyth_solana_receiver';
...
export function parseSwitchboardOracleV2(program, accountInfo, oracle) {
    try {
        //
...
export function parseSwitchboardOnDemandOracle(program, accountInfo, oracle) {
    try {
        ...
        const stdDeviation = getStandardDeviation(values.map((val) => new Big(val.value.toString()).div(1e18).toNumber()));
        return {
            price,
            lastUpdatedSlot,
...
export async function parseSwitchboardOracle(oracle, accountInfo, connection) {
    if (accountInfo.owner.equals(ON_DEMAND_MAINNET_PID)) {
        if (!sbOnDemandProgram) {
            const options = AnchorProvider.defaultOptions();
            const provider = new AnchorProvider(connection, null, options);
            const idl = await Anchor30Program.fetchIdl(ON_DEMAND_MAINNET_PID, provider);
            sbOnDemandProgram = new Anchor30Program(idl, provider);
        }
        return parseSwitchboardOnDemandOracle(sbOnDemandProgram, accountInfo, oracle);
    }
...
export function isOrcaOracle(accountInfo) {
    for (let i = 0; i < 8; i++) {
...
export function isRaydiumOracle(accountInfo) {
    for (let i = 0; i < 8; i++) {
...
export function isClmmOracle(accountInfo) {
    return isOrcaOracle(accountInfo) || isRaydiumOracle(accountInfo);
}
export function parsePythOracle(accountInfo, connection) {
    if (accountInfo.owner.equals(DEFAULT_RECEIVER_PROGRAM_ID)) {
        if (!pythSolanaReceiverProgram) {
            const options = AnchorProvider.defaultOptions();
            const provider = new AnchorProvider(connection, null, options);
            pythSolanaReceiverProgram = new Program(IDL, DEFAULT_RECEIVER_PROGRAM_ID, provider);
        }
        const decoded = pythSolanaReceiverProgram.coder.accounts.decode('priceUpdateV2', accountInfo.data);
        return {
            price: toUiDecimals(decoded.priceMessage.price.toNumber(), -decoded.priceMessage.exponent),
            publishedTime: decoded.priceMessage.publishTime.toNumber(),
            lastUpdatedSlot: decoded.postedSlot.toNumber(),
            uiDeviation: toUiDecimals(decoded.priceMessage.conf.toNumber(), -decoded.priceMessage.exponent),
...
export function isOracleStaleOrUnconfident(nowSlot, maxStalenessSlots, oracleLastUpdatedSlot, deviation, confFilter, price, debug = false, debugPrefix = undefined) {
    ...
    if (debug && deviation) {
        console.log(`- ${debugPrefix?.padStart(30)}: deviation within confidence tolerance ${deviation.div(price).mul(I80F48.fromNumber(100)).toFixed(3)}%`);
    }
    return false;
}
export function deriveFallbackOracleQuoteKey(accountInfo) {
...
export async function createFallbackOracleMap(conn, oracles, fallbacks) {
    const map = new Map();
