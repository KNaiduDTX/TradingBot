...
class PullFeed {
    ...
    constructor(program, pubkey) {
        ...
        this.gatewayUrl = "";
        this.pubkey = new anchor_1.web3.PublicKey(pubkey);
        this.configs = null;
        this.jobs = null;
    }
    ...
    static getPayer(program, payer) {
        var _a;
        return (_a = payer !== null && payer !== void 0 ? payer : program.provider.publicKey) !== null && _a !== void 0 ? _a : anchor_1.web3.PublicKey.default;
    }
    getPayer(payer) {
    ...
    initIx(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const lutSigner = (yield anchor_1.web3.PublicKey.findProgramAddress([buffer_1.Buffer.from("LutSigner"), this.pubkey.toBuffer()], this.program.programId))[0];
            const recentSlot = yield this.program.provider.connection.getSlot("finalized");
            const [_, lut] = anchor_1.web3.AddressLookupTableProgram.createLookupTable({
                authority: lutSigner,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ix = this.program.instruction.pullFeedSetConfigs({
                ...
                maxVariance: params.maxVariance !== undefined
                    ? new anchor_1.BN(Math.floor(params.maxVariance * 1e9))
                    : null,
    ...
    static fetchUpdateIx(params, debug, payer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { queue } = yield params.pullFeed.loadConfigs();
            ...
            const oracleResponses = report.oracle_responses.map((x) => {
                ...
                const oraclePubkey = isSolana
                    ? new anchor_1.web3.PublicKey(oraclePubkeyBytes)
                    : anchor_1.web3.PublicKey.findProgramAddressSync([buffer_1.Buffer.from("Oracle"), queue.toBuffer(), oraclePubkeyBytes], params.pullFeed.program.programId)[0];
    ...
    static fetchUpdateIxSvm(params, debug, payer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const jobs = yield crossbarClient
                ...
                .then((resp) => resp.jobs);
            const { responses, failures } = yield queue_js_1.Queue.fetchSignatures(feed.program, {
                gateway: params.gateway,
                numSignatures: params.numSignatures,
                jobs: jobs,
            ...
            const oracleResponses = responses.map((resp) => {
                ...
                const oraclePubkey = isSolana
                    ? new anchor_1.web3.PublicKey(oraclePubkeyBytes)
                    : anchor_1.web3.PublicKey.findProgramAddressSync([
    ...
    static fetchUpdateManyIx(program_1, params_1) {
        return __awaiter(this, arguments, void 0, function* (program, params, debug = false) {
            ...
            const queue = (_c = (_b = feedDatas[0]) === null || _b === void 0 ? void 0 : _b.queue) !== null && _c !== void 0 ? _c : anchor_1.web3.PublicKey.default;
            const feedConfigs = [];
            ...
            const instructionData = {
                slot: new anchor_1.BN(slotHashes[0][0]),
                values: response.median_responses.map(({ value }) => new anchor_1.BN(value)),
            };
            ...
            const feedPubkeys = response.median_responses.map((median_response) => {
                ...
                return anchor_1.web3.PublicKey.default;
            });
            ...
            const oraclePubkeys = response.oracle_responses.map((response) => {
                return new anchor_1.web3.PublicKey(buffer_1.Buffer.from(response.oracle_pubkey, "hex"));
            });
            const oracleFeedStatsPubkeys = oraclePubkeys.map((oracle) => anchor_1.web3.PublicKey.findProgramAddressSync([buffer_1.Buffer.from("OracleStats"), oracle.toBuffer()], program.programId)[0]);
    ...
    getSolanaSubmitSignaturesIx(params) {
        ...
        const isSolana = getIsSolana(params.chain);
        let queue = new anchor_1.web3.PublicKey(buffer_1.Buffer.from(resps[0].queue_pubkey.toString(), "hex"));
        const sourceQueueKey = new anchor_1.web3.PublicKey(buffer_1.Buffer.from(resps[0].queue_pubkey.toString(), "hex"));
        let queueBump = 0;
        if (!isSolana) {
            [queue, queueBump] = anchor_1.web3.PublicKey.findProgramAddressSync([buffer_1.Buffer.from("Queue"), queue.toBuffer()], program.programId);
        }
        const oracles = resps.map((x) => {
            const sourceOracleKey = new anchor_1.web3.PublicKey(buffer_1.Buffer.from(x.oracle_pubkey.toString(), "hex"));
            if (isSolana) {
                return sourceOracleKey;
            }
            else {
                const [oraclePDA] = anchor_1.web3.PublicKey.findProgramAddressSync([buffer_1.Buffer.from("Oracle"), queue.toBuffer(), sourceOracleKey.toBuffer()], program.programId);
                return oraclePDA;
            }
        });
        const oracleFeedStats = oracles.map((oracle) => anchor_1.web3.PublicKey.findProgramAddressSync([buffer_1.Buffer.from("OracleStats"), oracle.toBuffer()], program.programId)[0]);
        const submissions = resps.map((resp, idx) => ({
            value: new anchor_1.BN(resp.success_value.toString()),
        ...
        const instructionData = {
            slot: new anchor_1.BN(params.slot),
            submissions: submissions.map((x) => {
                x.signature = buffer_1.Buffer.from(x.signature, "base64");
                return x;
            }),
    ...
    loadValues() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return data.submissions
                .filter((x) => !x.oracle.equals(anchor_1.web3.PublicKey.default))
                .map((x) => {
                common_1.Big.DP = 40;
                return {
                    value: new common_1.Big(x.value.toString()).div(1e18),
                    slot: new anchor_1.BN(x.slot.toString()),
                    oracle: new anchor_1.web3.PublicKey(x.oracle),
                };
            });
        });
    }
    ...
    lookupTableKey(data) {
        const lutSigner = anchor_1.web3.PublicKey.findProgramAddressSync([buffer_1.Buffer.from("LutSigner"), this.pubkey.toBuffer()], this.program.programId)[0];
        const [_, lutKey] = anchor_1.web3.AddressLookupTableProgram.createLookupTable({
            authority: lutSigner,
            payer: anchor_1.web3.PublicKey.default,
    ...
    loadHistoricalValuesCompact(data_) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const values = data.historicalResults
                .filter((x) => x.slot.gt(new anchor_1.BN(0)))
