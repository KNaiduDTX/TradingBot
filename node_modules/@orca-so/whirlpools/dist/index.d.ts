...
import { Address, TransactionSigner, KeyPairSigner, IInstruction, Lamports, Rpc, GetAccountInfoApi, GetMultipleAccountsApi, GetMinimumBalanceForRentExemptionApi, GetEpochInfoApi, Signature, GetProgramAccountsApi, Account, GetTokenAccountsByOwnerApi } from '@solana/kit';
...
export { getRpcConfig, setComputeUnitMarginMultiplier, setJitoBlockEngineUrl, setJitoFeePercentile, setJitoTipSetting, setPriorityFeePercentile, setPriorityFeeSetting, setRpc } from '@orca-so/tx-sender';
...
declare function setPayerFromBytes(pkBytes: Uint8Array<ArrayBuffer>): Promise<KeyPairSigner<string>>;
declare function getPayer(): KeyPairSigner;
...
declare const createSplashPool: (tokenMintA: Address, tokenMintB: Address, initialPrice?: number | undefined) => Promise<CreatePoolInstructions & {
    callback: () => Promise<_solana_kit.Signature>;
}>;
declare const createConcentratedLiquidityPool: (tokenMintA: Address, tokenMintB: Address, tickSpacing: number, initialPrice?: number | undefined) => Promise<CreatePoolInstructions & {
    callback: () => Promise<_solana_kit.Signature>;
}>;
...
declare const closePosition: (positionMintAddress: Address, slippageToleranceBps?: number | undefined) => Promise<DecreaseLiquidityInstructions & {
    feesQuote: CollectFeesQuote;
    rewardsQuote: CollectRewardsQuote;
} & {
    callback: () => Promise<_solana_kit.Signature>;
}>;
declare const decreaseLiquidity: (positionMintAddress: Address, param: DecreaseLiquidityQuoteParam, slippageToleranceBps?: number | undefined) => Promise<DecreaseLiquidityInstructions & {
    callback: () => Promise<_solana_kit.Signature>;
}>;
...
declare const harvestPosition: (positionMintAddress: Address) => Promise<HarvestPositionInstructions & {
    callback: () => Promise<Signature>;
}>;
declare function harvestAllPositionFees(): Promise<Signature[]>;
...
declare const increasePosLiquidity: (positionMintAddress: Address, param: IncreaseLiquidityQuoteParam, slippageToleranceBps?: number | undefined) => Promise<IncreaseLiquidityInstructions & {
    callback: () => Promise<_solana_kit.Signature>;
}>;
declare const openFullRangePosition: (poolAddress: Address, param: IncreaseLiquidityQuoteParam, slippageToleranceBps?: number | undefined) => Promise<IncreaseLiquidityInstructions & {
    initializationCost: Lamports;
    positionMint: Address;
} & {
    callback: () => Promise<_solana_kit.Signature>;
}>;
declare const openConcentratedPosition: (poolAddress: Address, param: IncreaseLiquidityQuoteParam, lowerPrice: number, upperPrice: number, slippageToleranceBps?: number | undefined) => Promise<IncreaseLiquidityInstructions & {
    initializationCost: Lamports;
    positionMint: Address;
} & {
    callback: () => Promise<_solana_kit.Signature>;
...
declare const swap: <T extends SwapParams>(params: T, poolAddress: Address, slippageToleranceBps?: number | undefined) => Promise<SwapInstructions<T> & {
    callback: () => Promise<_solana_kit.Signature>;
}>;

declare function orderMints(mint1: Address, mint2: Address): [Address, Address];

export { type ClosePositionInstructions, type CreatePoolInstructions, DEFAULT_ADDRESS, DEFAULT_FUNDER, DEFAULT_NATIVE_MINT_WRAPPING_STRATEGY, DEFAULT_SLIPPAGE_TOLERANCE_BPS, DEFAULT_WHIRLPOOLS_CONFIG_ADDRESSES, DEFAULT_WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS, type DecreaseLiquidityInstructions, type DecreaseLiquidityQuoteParam, type ExactInParams, type ExactOutParams, FUNDER, type HarvestPositionInstructions, type HydratedPosition, type HydratedPositionBundle, type IncreaseLiquidityInstructions, type IncreaseLiquidityQuoteParam, type InitializablePool, type InitializedPool, NATIVE_MINT_WRAPPING_STRATEGY, type NativeMintWrappingStrategy, type OpenPositionInstructions, type PoolInfo, type PositionData, type PositionOrBundle, SLIPPAGE_TOLERANCE_BPS, SPLASH_POOL_TICK_SPACING, type SwapInstructions, type SwapParams, type SwapQuote, WHIRLPOOLS_CONFIG_ADDRESS, WHIRLPOOLS_CONFIG_EXTENSION_ADDRESS, closePosition, closePositionInstructions, createConcentratedLiquidityPool, createConcentratedLiquidityPoolInstructions, createSplashPool, createSplashPoolInstructions, decreaseLiquidity, decreaseLiquidityInstructions, fetchConcentratedLiquidityPool, fetchPositionsForOwner, fetchPositionsInWhirlpool, fetchSplashPool, fetchWhirlpoolsByTokenPair, getPayer, harvestAllPositionFees, harvestPosition, harvestPositionInstructions, increaseLiquidityInstructions, increasePosLiquidity, openConcentratedPosition, openFullRangePosition, openFullRangePositionInstructions, openPositionInstructions, orderMints, resetConfiguration, setDefaultFunder, setDefaultSlippageToleranceBps, setNativeMintWrappingStrategy, setPayerFromBytes, setWhirlpoolsConfig, swap, swapInstructions };
